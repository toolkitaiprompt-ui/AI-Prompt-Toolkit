...
function readabilityScore(text: string): number {
  ...
  if (words === 0) return 0;
  const syllables = Math.ceil(chars / 4.7);
...
function wordDiff(textA: string, textB: string): { tokens: DiffToken[]; added: number; removed: number } {
  ...
  while (i > 0 || j > 0) {
    if (i > 0 && j > 0 && wordsA[i - 1] === wordsB[j - 1]) {
      tokens.unshift({ text: wordsA[i - 1], type: "same" }); i--; j--;
    } else if (j > 0 && (i === 0 || dp[i][j - 1] >= dp[i - 1][j])) {
      tokens.unshift({ text: wordsB[j - 1], type: "added" }); added++; j--;
    } else {
      tokens.unshift({ text: wordsA[i - 1], type: "removed" }); removed++; i--;
    }
  }
    ...
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-xs">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`font-bold ${same ? "text-slate-300" : improved ? "text-emerald-400" : "text-red-400"}`}>{valueB}{suffix}</span>
          {!same && <span className={`text-[10px] ${improved ? "text-emerald-500" : "text-red-500"}`}>{improved ? "+" : ""}{diff}{suffix}</span>}
        </div>
      </div>
      <div className="flex items-center gap-1">
        ...
        <div className="h-1.5 flex-1 rounded-full bg-slate-800 overflow-hidden">
          <div className={`h-full rounded-full ${same ? "bg-slate-600" : improved ? "bg-emerald-500" : "bg-red-500"}`} style={{ width: `${Math.min(100, valueB)}%` }} />
        </div>
      </div>
    </div>
  );
...
export default function PromptComparison() {
  ...
  const analysis = useMemo(() => {
    ...
    const a = { tokens: estimateTokens(promptA), words: countWords(promptA), chars: countChars(promptA), readability: readabilityScore(promptA), structure: structureScore(promptA), clarity: clarityScore(promptA) };
    const b = { tokens: estimateTokens(promptB), words: countWords(promptB), chars: countChars(promptB), readability: readabilityScore(promptB), structure: structureScore(promptB), clarity: clarityScore(promptB) };
    ...
    const overall = Math.round((structDelta * 0.4 + clarityDelta * 0.4 + readabilityDelta * 0.2) + 50);

    const diff = wordDiff(promptA, promptB);

    // Determine winner
    const aAvg = (a.structure + a.clarity + a.readability) / 3;
    const bAvg = (b.structure + b.clarity + b.readability) / 3;
    let winner: "A" | "B" | "tie" = "tie";
    if (bAvg > aAvg + 3) winner = "B";
    else if (aAvg > bAvg + 3) winner = "A";

    // Why better
    const whyBetter: string[] = [];
    if (b.structure > a.structure + 10) whyBetter.push("Better structure — includes role, format, or constraints that guide the AI.");
    if (b.clarity > a.clarity + 10) whyBetter.push("Higher clarity — more specific words and clearer instructions reduce ambiguity.");
    if (b.readability > a.readability + 10) whyBetter.push("More readable — sentence length and complexity are better optimized.");
    if (diff.added > diff.removed) whyBetter.push(`Adds ${diff.added} new elements that provide more context or specificity.`);
    if (diff.removed > diff.added && diff.removed > 3) whyBetter.push("Removes unnecessary words, making the prompt more concise.");
    if (b.words > a.words + 20) whyBetter.push("More detailed — adds additional instructions for better AI output.");
    if (a.words > b.words + 20 && b.words > 10) whyBetter.push("More concise — retains key information in fewer words.");
    if (whyBetter.length === 0) {
      if (bAvg > aAvg) whyBetter.push("Scores slightly higher across quality metrics.");
      else if (aAvg > bAvg) whyBetter.push("Scores slightly higher across quality metrics.");
      else whyBetter.push("Both prompts score similarly. Minor differences in structure and wording.");
    }

    return { a, b, overall, diff, winner, whyBetter };
    ...
    <div className="space-y-6">
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="space-y-2">
          ...
          <textarea className="h-48 w-full rounded-xl border border-slate-700 bg-slate-900 p-3 text-sm text-slate-100 outline-none transition focus:border-red-400/60 focus:ring-2 focus:ring-red-400/20" value={promptA} onChange={(e) => { setPromptA(e.target.value); setCompared(false); }} placeholder="Paste your original prompt here..." aria-label="Prompt A input" />
        </div>
        <div className="space-y-2">
          <div className="flex items-center justify-between">
          ...
          <textarea className="h-48 w-full rounded-xl border border-slate-700 bg-slate-900 p-3 text-sm text-slate-100 outline-none transition focus:border-emerald-400/60 focus:ring-2 focus:ring-emerald-400/20" value={promptB} onChange={(e) => { setPromptB(e.target.value); setCompared(false); }} placeholder="Paste your optimized prompt here..." aria-label="Prompt B input" />
        </div>
      </div>
      ...
      {analysis && (
        <div className="space-y-6">
          {/* ── Winner Badge ── */}
          {analysis.winner && (
            <div className={`rounded-2xl border p-5 text-center ${analysis.winner === "B" ? "border-emerald-400/30 bg-emerald-500/10" : analysis.winner === "A" ? "border-amber-400/30 bg-amber-500/10" : "border-slate-600/30 bg-slate-800/30"}`}>
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Winner</p>
              <p className={`text-2xl font-black ${analysis.winner === "B" ? "text-emerald-400" : analysis.winner === "A" ? "text-amber-400" : "text-slate-400"}`}>
                {analysis.winner === "B" ? "🏆 Prompt B" : analysis.winner === "A" ? "🏆 Prompt A" : "🤝 Tie"}
              </p>
              <p className="text-xs text-slate-500 mt-1">
                {analysis.winner === "B" ? "Prompt B scores higher across readability, structure, and clarity." :
                 analysis.winner === "A" ? "Prompt A scores higher across readability, structure, and clarity." :
                 "Both prompts score similarly."}
              </p>
            </div>
          )}

          {/* ── Why Better ── */}
          {analysis.whyBetter && analysis.whyBetter.length > 0 && (
            <div className="rounded-2xl border border-white/10 bg-slate-900/50 p-5">
              <h3 className="text-sm font-semibold text-white mb-3">💡 Why Prompt {analysis.winner === "B" ? "B" : analysis.winner === "A" ? "A" : "A/B"} Is Better</h3>
              <ul className="space-y-2">
                {analysis.whyBetter.map((reason, i) => (
          ...
          <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900 to-slate-950 p-6 text-center">
            <div className="flex items-center justify-center gap-2 text-sm text-slate-400">
              <TrendingUp className="h-4 w-4" /> Overall Improvement
            </div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
            <div className="mb-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3 text-xs">
                <span className="flex items-center gap-1 text-emerald-400"><span className="inline-block h-2 w-2 rounded-full bg-emerald-500" /> Added ({analysis.diff.added})</span>
                <span className="flex items-center gap-1 text-red-400"><span className="inline-block h-2 w-2 rounded-full bg-red-500" /> Removed ({analysis.diff.removed})</span>
              </div>
            </div>
            <div className="rounded-xl bg-slate-950 p-4 text-sm leading-7">
              {analysis.diff.tokens.map((token, idx) => (
                <span key={idx} className={token.type === "added" ? "bg-emerald-500/20 text-emerald-300 rounded px-1" : token.type === "removed" ? "bg-red-500/20 text-red-300 line-through rounded px-1" : "text-slate-400"}>{token.text} </span>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className={`rounded-xl border p-4 ${analysis.winner === "A" ? "border-emerald-500/30 bg-emerald-500/10" : "border-red-500/20 bg-red-500/5"}`}>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-bold text-red-400">PROMPT A</p>
                  {analysis.winner === "A" && <span className="text-[10px] font-semibold text-emerald-400 bg-emerald-500/20 px-2 py-0.5 rounded-full">🏆 Winner</span>}
                </div>
              ...
              <div className={`rounded-xl border p-4 ${analysis.winner === "B" ? "border-emerald-500/30 bg-emerald-500/10" : "border-emerald-500/20 bg-emerald-500/5"}`}>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-bold text-emerald-400">PROMPT B</p>
                  {analysis.winner === "B" && <span className="text-[10px] font-semibold text-emerald-400 bg-emerald-500/20 px-2 py-0.5 rounded-full">🏆 Winner</span>}
                </div>
                <ul className="space-y-1 text-xs text-slate-400">
