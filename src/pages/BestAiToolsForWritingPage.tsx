...
export default function BestAiToolsForWritingPage() {
  useEffect(() => {
    const seo = getSeoForPath("/best-ai-tools-for-writing");
    ...
    ensureProp("og:url").setAttribute("content", toCanonical("/best-ai-tools-for-writing"));

    const canonical = document.querySelector('link[rel="canonical"]');
    if (canonical) {
      canonical.setAttribute("href", toCanonical("/best-ai-tools-for-writing"));
    }

    // Hreflang
    document.querySelectorAll('link[rel="alternate"][hreflang]').forEach((el) => el.remove());
    const hreflangs = ["en", "en-US", "en-GB", "en-IN", "x-default"];
    const canonicalUrl = toCanonical("/best-ai-tools-for-writing");
  ...
  const promptPrompts = [
    ...
    "/prompts/seo-specialist",
    "/prompts/entrepreneur",
    "/prompts/marketer",
  ];
    ...
    <section className="site-container section-lg">
      ...
      >
        ...
        <h1 className="font-headline font-bold text-4xl sm:text-5xl text-white mb-4">
          Best AI Tools for Writing & Content Creation 2026
        </h1>
        <p className="text-slate-400 max-w-2xl mx-auto">
          Discover the best AI tools for writing and content creation. Compare features, pricing, and use cases to find the right tool for your projects.
        </p>
      </motion.div>
        ...
        <form onSubmit={handleSearch} className="bg-slate-900/80 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 max-w-xl mx-auto">
          <div className="grid grid-cols-1 gap-2 mb-4">
            <input
              ...
              aria-label="Search AI writing tools"
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        {TOOL_PAGES.map((tool) => {
          if (
            !(
              tool.path === "/tools/prompt-variable-extractor" ||
              tool.path === "/tools/prompt-formatter" ||
              tool.path === "/tools/prompt-debugger" ||
              tool.path === "/tools/content-summarizer" ||
              tool.path === "/tools/advanced-prompt-optimizer"
            )
          )
            return null;
          return (
      ...
      <div className="mt-12 pt-12 border-t border-white/10">
        <h3 className="font-bold text-white mb-4">
          AI Prompts for Writing Projects
        </h3>
        <p className="text-slate-400 mb-6">
          Ready-to-use prompts for common writing tasks. Copy, paste, and get professional results instantly.
        </p>
        <div className="grid grid-cols-2 gap-4">
          {promptPrompts.map((path) => {
